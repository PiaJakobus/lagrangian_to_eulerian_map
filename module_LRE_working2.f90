MODULE LRE
  ...
  SUBROUTINE get_grid_value(i_g, INT_param, rdim, N_variable,zaehler,zaehler0, &
   ...
   DOUBLE PRECISION, INTENT(IN)    :: rho_red(N), pmass_red(N), h_red(N), r_particles(rdim,N)
   DOUBLE PRECISION, INTENT(OUT)   :: f_grid_vec(N_grid, N_variable)
   DOUBLE PRECISION, INTENT(INOUT) :: f_grid,moment_matrix(INT_param,INT_param), B(INT_param)
   LOGICAL, INTENT(IN)             :: polar
  ...
  SUBROUTINE to_polar(r,rdim,N_variable,c_light,dist,f_grid_vec_i,f_grid_pol)
    ...
    DOUBLE PRECISION :: x,y,z,xy
    DOUBLE PRECISION :: vx,vy,vz
    DOUBLE PRECISION,INTENT(IN) :: f_grid_vec_i(N_variable)
    DOUBLE PRECISION,INTENT(OUT) :: f_grid_pol(N_variable)

    vx = f_grid_vec_i(2) * c_light
    vy = f_grid_vec_i(3) * c_light
    vz = f_grid_vec_i(4) * c_light 
    x = r(1) * 100000.D0 * sin(r(2)) * cos(r(3))
    y = r(1) * 100000.D0 * sin(r(2)) * sin(r(3))
    z = r(1) * 100000.D0 * cos(r(2))
    xy = x**2.d0 + y**2.d0
    dist = SQRT(xy + z**2.d0)
    f_grid_pol(1) = f_grid_vec_i(1)
    f_grid_pol(2) = (x*vx + y*vy + z*vz) / dist
    f_grid_pol(4) = (z*f_grid_pol(2) - dist*vz) / (dist**2.D0 * SIN(r(2)))
    f_grid_pol(3) = (x*vy - y*vx) / xy
    f_grid_pol(5) = ABS(1.0D0 - (f_grid_vec_i(5) / (dist *1e-5)))
    !print*, f_grid_pol(5)
