MODULE LRE
  ...
  CONTAINS
     ...
     SUBROUTINE assemble_moment_matrix(N,INT_param,moment_matrix,rp,r_G,rdim,h,rho,m,polar)
  ...
  PURE SUBROUTINE get_vector_b(B,fp,N,rp,r_G,rdim,INT_param,h,rho,m,polar)
  ...
  PURE SUBROUTINE distance_polar(xp,rg,dist)
    ...
    x = rg(1) * sin(rg(3)) * cos(rg(2))
    y = rg(1) * sin(rg(3)) * sin(rg(2))
    z = rg(1) * cos(rg(3))
    dist = SQRT((xp(1)-x)**2.d0 + (xp(2)-y)**2.d0 + (xp(3)-z)**2.d0)
  END SUBROUTINE distance_polar


  SUBROUTINE standard_SPH(fp,N,rp,r_G,rdim,h,rho,m,f_grid,polar)
  ...
  PURE SUBROUTINE get_base3D_lin(pos,pos0,bases_3D_lin,polar)
    ...
    IF (polar .eqv. .true.) THEN 
      x(1) = pos0(1) * sin(pos0(3)) * cos(pos0(2))
      x(2) = pos0(1) * sin(pos0(3)) * sin(pos0(2))
      x(3) = pos0(1) * cos(pos0(3))
    END IF 
  ...
  PURE SUBROUTINE get_base3D_quad(pos,pos0,bases_3D_quad,polar)
